...
# ==========================================
# 5. VISUALIZATION (COMBINED)
# ==========================================
num_snapshots = len(history)
print(f"\nSimulation finished. Processing {num_snapshots} snapshots...")

# --- PART A: Save the Static "Film Strip" Image ---
print("1. Generating static snapshots image...")

# Create a wide figure to hold all snapshots side-by-side
fig_static, axes = plt.subplots(1, num_snapshots, figsize=(3 * num_snapshots, 4))

# Handle edge case if there's only 1 plot (prevents crash)
if num_snapshots == 1: axes = [axes]

for i, ax in enumerate(axes):
    current_data = history[i]
    ax.imshow(current_data, cmap='Greys', origin='lower', vmin=-1.1, vmax=1.1)
...
plt.suptitle(f"Simulation History (Total Steps: {t})", fontsize=16)

# Save the static image file
plt.savefig("simulation_snapshots.png", dpi=150, bbox_inches='tight')
print("   ✅ Saved 'simulation_snapshots.png'")

# Close the figure to clear memory before making the GIF
plt.close(fig_static) 


# --- PART B: Generate the GIF Animation ---
print("2. Generating GIF animation...")

# Create a new, square figure for the animation
fig_anim, ax = plt.subplots(figsize=(6, 6))

# Setup the first frame (Step 0)
im = ax.imshow(history[0], cmap='Greys', origin='lower', vmin=-1.1, vmax=1.1)
title = ax.set_title("Step 0")
fig_anim.colorbar(im, ax=ax, label=r"Concentration ($\phi$)")

def update(frame_idx):
    # Update the image data to the next snapshot in history
    im.set_array(history[frame_idx])
    
    # Calculate current step for the title
    # (If it's the last frame, we use the final step count 't')
    if frame_idx == len(history) - 1:
        current_step = t
    else:
        current_step = frame_idx * plot_interval
        
    title.set_text(f"Simulation Step: {current_step}")
    return [im, title]

# Create the animation
ani = animation.FuncAnimation(fig_anim, update, frames=len(history), interval=200, blit=False)
#Changing the interval increases time between frames


# Save the GIF
ani.save('simulation_movie.gif', writer='pillow', fps=5)
print("   ✅ Saved 'simulation_movie.gif'")

# Optional: Show the final window if you want to see it live
plt.show()